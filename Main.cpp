...
struct detours
{
	utils::hook::detour Live_GetUserData_h;
	utils::hook::detour LiveStorage_StatsInit_h;
	utils::hook::detour isPremiumPlayerReal_h;
...
bool g_shouldHookLiveGetUserData = false;
//++++++++++++++++++++++++++++++
// en : LUA Status
// ja : LUA状態
//++++++++++++++++++++++++++++++
...
//++++++++++++++++++++++++++++++
// en : In-game address calculation (CORRECTED)
// ja : ゲーム内アドレス算出 (修正版)
//++++++++++++++++++++++++++++++
size_t CalcPtr(const size_t dump_address)
{
	// Vérification que dump_address est valide
	if (dump_address == 0) {
		return 0;
	}

	// Vérification que DumpBase est valide
	if (_adr.DumpBase == 0) {
		return 0;
	}

	// Vérification que l'adresse dump est bien dans la plage attendue
	if (dump_address < _adr.DumpBase) {
		return 0;
	}

	// Calcul de l'offset relatif
	uintptr_t relative_offset = dump_address - _adr.DumpBase;

	// Calcul de l'adresse finale
	uintptr_t final_address = _ImageBase + relative_offset;

	// Vérification que l'adresse finale est dans la plage du module
	if (final_address < _ImageBase) {
		return 0;
	}

	// Vérification que l'adresse finale n'est pas trop loin
	if (final_address > _ImageBase + 0x20000000) {
		return 0;
	}
...
// Version avec vérification de lecture (optionnel)
size_t CalcPtr_Verified(const size_t dump_address)
{
	uintptr_t addr = CalcPtr(dump_address);
	if (addr == 0) return 0;
...
// Version template pour différents types
template<typename T>
T* CalcPtr_T(const size_t dump_address)
{
	return reinterpret_cast<T*>(CalcPtr(dump_address));
}
...
// Fonction callback pour la nouvelle adresse
...
void GetAddressOffset(GameTitle title)
{
	...
	case GameTitle::IW8_157:
	{
		...
		_adr.R_EndFrame = 0x7FF6B34B5DC0;
		_adr.Dvar_RegisterBool = 0x7FF6B0D603C0;
		NotifyMsg("[Debug] DumpBase: 0x%llx\n", _adr.DumpBase);
		NotifyMsg("[Debug] ImageBase: 0x%llx\n", _ImageBase);

		uintptr_t testAddr = CalcPtr(_adr.R_EndFrame);
		NotifyMsg("[Debug] R_EndFrame calculated: 0x%llx\n", testAddr);

		// COMMENTEZ TEMPORAIREMENT la vérification qui fait crasher
		/*
		if (testAddr > _ImageBase && testAddr < _ImageBase + 0x4000000) {
			__try {
				volatile int test = *(int*)testAddr;
				NotifyMsg("[Debug] Address is readable! Value: 0x%x\n", test);
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] Address is NOT readable\n");
			}
		}
		else {
			NotifyMsg("[ERROR] Calculated address 0x%llx is OUT OF RANGE!\n", testAddr);
		}
		*/
		NotifyMsg("[DEBUG] LUI_CoD_LuaCall_OfflineDataFetched dump addr: 0x%llx\n", _adr.LUI_CoD_LuaCall_OfflineDataFetched);
		uintptr_t testOffline = CalcPtr(_adr.LUI_CoD_LuaCall_OfflineDataFetched);
		NotifyMsg("[DEBUG] LUI_CoD_LuaCall_OfflineDataFetched calculated: 0x%llx\n", testOffline);

		NotifyMsg("[DEBUG] LUI_COD_LuaCall_IsPremiumPlayer dump addr: 0x%llx\n", _adr.LUI_COD_LuaCall_IsPremiumPlayer);
		uintptr_t testPremium = CalcPtr(_adr.LUI_COD_LuaCall_IsPremiumPlayer);
		NotifyMsg("[DEBUG] LUI_COD_LuaCall_IsPremiumPlayer calculated: 0x%llx\n", testPremium);
		// Dans GetAddressOffset(), après avoir défini les adresses
		NotifyMsg("[DEBUG] LUI_OfflineDataFetched dump: 0x%llx -> calc: 0x%llx\n",
			_adr.LUI_CoD_LuaCall_OfflineDataFetched,
			CalcPtr(_adr.LUI_CoD_LuaCall_OfflineDataFetched));

		NotifyMsg("[DEBUG] IsPremiumPlayer dump: 0x%llx -> calc: 0x%llx\n",
			_adr.LUI_COD_LuaCall_IsPremiumPlayer,
			CalcPtr(_adr.LUI_COD_LuaCall_IsPremiumPlayer));

		NotifyMsg("[DEBUG] IsLocalPlayAllowed dump: 0x%llx -> calc: 0x%llx\n",
			_adr.LUI_CoD_LuaCall_IsLocalPlayAllowed,
			CalcPtr(_adr.LUI_CoD_LuaCall_IsLocalPlayAllowed));
...
void TestLuaFunctionReturns() {
	...
	struct TestFunc {
		...
		{"IsConnectedToGameServer", _adr.LUI_CoD_LuaCall_IsConnectedToGameServer},
		{"IsGameModeAllowed", _adr.LUI_CoD_LuaCall_IsGameModeAllowed},
		{"IsGameModeAvailable", _adr.LUI_CoD_LuaCall_IsGameModeAvailable},
		{"IsPremiumPlayerReady", _adr.LUI_COD_LuaCall_IsPremiumPlayerReady},
		{"IsUserSignedInToDemonware", _adr.LUI_CoD_LuaCall_IsUserSignedInToDemonware},
		{"OfflineDataFetched", _adr.LUI_CoD_LuaCall_OfflineDataFetched},
		{"IsPremiumPlayer", _adr.LUI_COD_LuaCall_IsPremiumPlayer}
	};

	for (int i = 0; i < sizeof(testFuncs) / sizeof(testFuncs[0]); i++) {
		uintptr_t addr = CalcPtr(testFuncs[i].offset);
		if (addr > _ImageBase && addr < _ImageBase + 0x4000000) {
			__try {
				BoolFunc func = (BoolFunc)addr;
				bool result = func();
				NotifyMsg("[TEST] %s returned: %s\n", testFuncs[i].name, result ? "true" : "false");
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] Cannot execute %s\n", testFuncs[i].name);
			}
		}
	}
}
...
int lua_print_d(lua_State* s)
{
	int n = 0;
	// Récupérer le nombre d'arguments (difficile sans lua_gettop)
	// On va lire jusqu'à trouver nil
	std::string output = "[Lua] ";

	for (int i = 1; i <= 10; i++) { // Max 10 arguments
		// Utiliser lua_tolstring que vous avez déjà
		const char* str = lua_tolstring(s, i, nullptr);
		if (str != nullptr) {
			if (i > 1) output += "\t";
			output += str;
			n++;
			...
			// Vérifier si c'est un nombre (via lua_type si disponible)
			// Sinon on s'arrête
			break;
		}
	}
...
//++++++++++++++++++++++++++++++
void LUI_ReportError_d(const char* error) {
	NotifyMsg("LUI Error bloquée : %s\n", error);
	if (strstr(error, "stats") || strstr(error, "data")) {
		return; // On ignore l'erreur pour empêcher le crash
	}
	// Sinon, laisser passer ou logger
}
...
//++++++++++++++++++++++++++++++
// en : Inject Lua script for weapon fixes
// ja : 武器修正のLuaスクリプトを注入
//++++++++++++++++++++++++++++++
//++++++++++++++++++++++++++++++
// en : Inject Lua scripts for specific files
// ja : 特定のファイル用のLuaスクリプトを注入
//++++++++++++++++++++++++++++++
...
//++++++++++++++++++++++++++++++
// Fonction helper avec seulement du C pur (pas d'objets C++)
bool PatchDvarValue(const char* dvar_name, bool& value_patched)
{
	// Utiliser des chaînes C pures, pas de std::string
	if (dvar_name == nullptr) return false;

	// Comparaisons directes avec strcmp
	if (strcmp(dvar_name, "LSSRRSMNMR") == 0) {
		value_patched = true;
		NotifyMsg("[Success] <Dvar_RegisterBool> Patched lui_dev_features_enabled -> true\n");
		return true;
	}
	else if (strcmp(dvar_name, "LSTQOKLTRN") == 0) {
		value_patched = true;
		NotifyMsg("[Success] <Dvar_RegisterBool> Patched force_offline_menus -> true\n");
		return true;
	}
	else if (strcmp(dvar_name, "LLOKQOSPPP") == 0) {
		value_patched = true;
		NotifyMsg("[Success] <Dvar_RegisterBool> Patched xblive_loggedin -> true\n");
		return true;
	}
	else if (strcmp(dvar_name, "LLPNKKORPT") == 0) {
		value_patched = true;
		NotifyMsg("[Success] <Dvar_RegisterBool> Patched systemlink_host -> true\n");
		return true;
	}
	else if (strcmp(dvar_name, "LPSPMQSNPQ") == 0) {
		value_patched = true;
		NotifyMsg("[Success] <Dvar_RegisterBool> Patched systemlink -> true\n");
		return true;
	}
...
// Version principale avec gestion d'exceptions SEH
dvar_t* Dvar_RegisterBool_d(const char* dvar_name, bool value, unsigned int flags, const char* description)
{
	// Vérification de base
	if (!dvar_name) {
		return _hooks.Dvar_RegisterBool_h.stub<dvar_t*>(dvar_name, value, flags, description);
	}

	// Utiliser __try ici car la fonction appelée ne contient pas d'objets C++
	__try {
		bool value_patched = value;

		// Appeler la fonction helper (pas d'objets C++ dans le __try)
		PatchDvarValue(dvar_name, value_patched);

		// Appel du stub original
		return _hooks.Dvar_RegisterBool_h.stub<dvar_t*>(dvar_name, value_patched, flags, description);
	}
	__except (EXCEPTION_EXECUTE_HANDLER) {
		NotifyMsg("[EXCEPTION] <Dvar_RegisterBool> Crash prevented for '%s'\n", dvar_name);
		return _hooks.Dvar_RegisterBool_h.stub<dvar_t*>(dvar_name, value, flags, description);
	}
}
...
//++++++++++++++++++++++++++++++
// en : Loading a script file (for detour) - CORRECTED VERSION
// ja : スクリプトファイルをロードする ( ディトール用 ) - 修正版
//++++++++++++++++++++++++++++++
void Load_ScriptFile_d(DBStreamStart streamStart)
{
	// Appel à la fonction originale - charge le GSC normal du jeu
	_hooks.Load_ScriptFile_h.stub<void>(streamStart);

	// ========================================================================================== //
	// OPTIONNEL : UNIQUEMENT POUR DUMPER (si vous voulez sauvegarder les GSC)
	// ========================================================================================== //

	if (_dumpGSC)
	{
		// Récupérer le script file actuel
		ScriptFile** varScriptFile = reinterpret_cast<ScriptFile**>(CalcPtr(_adr.varScriptFile));

		if (varScriptFile && *varScriptFile)
		{
			ScriptFile* scriptfile = *varScriptFile;

			// Créer le chemin de dump
			std::string scriptFileStr = scriptfile->name;
			if (!isSubStr(scriptfile->name, ".gsc"))
			{
				scriptFileStr += ".gsc";
			}

			std::string dumpPath = _documentPath + "\\dumpdata\\gsc\\" + scriptFileStr + "bin";
			ReplaceAll(dumpPath, "/", "\\");

			// Créer le dossier si nécessaire
			size_t lastSlash = dumpPath.find_last_of("\\");
			if (lastSlash != std::string::npos)
			{
				std::string directory = dumpPath.substr(0, lastSlash);
				std::filesystem::create_directories(directory);
			}

			// Dumper le GSC
			std::ofstream gscbin_file(dumpPath, std::ios::out | std::ios::binary);
			if (gscbin_file.is_open())
			{
				dump_gsc_script(gscbin_file, scriptfile);
				gscbin_file.close();
				NotifyMsg("[Success] <Load_ScriptFile> GSC dumped: %s\n", dumpPath.c_str());
			}
		}
	}

	// ========================================================================================== //
	// PAS D'INJECTION - Les GSC originaux sont déjà chargés par le stub ci-dessus
	// ========================================================================================== //
...
bool IsInSystemLinkMenu()
{
	static int lastCheck = 0;
	static bool cachedResult = false;

	// DEBUG - logger à chaque appel
	NotifyMsg("[DEBUG] IsInSystemLinkMenu CHECK - frame:%d, lastCheck:%d, diff:%d\n",
		_elapsedFrameCount, lastCheck, abs(_elapsedFrameCount - lastCheck));

	if (abs(_elapsedFrameCount - lastCheck) < 60) {
		NotifyMsg("[DEBUG] IsInSystemLinkMenu CACHED - returning %d\n", cachedResult);
		return cachedResult;
	}

	lastCheck = _elapsedFrameCount;

	__try {
		NotifyMsg("[DEBUG] IsInSystemLinkMenu EXECUTING check...\n");

		if (_hooks.Live_IsInSystemlinkLobby_h.get_original()) {
			int result = Live_IsInSystemlinkLobby_d();
			NotifyMsg("[DEBUG] Live_IsInSystemlinkLobby_d returned: %d\n", result);

			if (result == 1) {
				NotifyMsg("[DEBUG] DETECTED SystemLink lobby!\n");
				cachedResult = true;
				return true;
			}
		}
	...
	__except (EXCEPTION_EXECUTE_HANDLER) {
		NotifyMsg("[DEBUG] EXCEPTION in IsInSystemLinkMenu\n");
	}

	cachedResult = false;
	NotifyMsg("[DEBUG] IsInSystemLinkMenu = false\n");
	return false;
}
...
{
	NotifyMsg("[Notice] <LiveStorage_BeginGame> detour called for client %d\n",
		localClientNum == LocalClientNum_t::LOCAL_CLIENT_0 ? 0 : 1);

	// 1. Ne pas appeler _hooks.LiveStorage_BeginGame_h.stub si cela cause un crash.
	// Dans beaucoup de versions IW8, on laisse la fonction originale s'exécuter 
	// SEULEMENT SI les stats sont déjà marquées comme "Offline" ou "Ready".

	// 2. Forcer l'état des stats pour éviter que le moteur Lua ne bloque
	// On utilise souvent une fonction interne pour marquer les stats comme prêtes.
	// Si vous avez l'adresse de LiveStorage_StatsInit, appelez-la :
	if (_adr.LiveStorage_StatsInit) {
		auto StatsInit = reinterpret_cast<void(*)(LocalClientNum_t)>(CalcPtr(_adr.LiveStorage_StatsInit));
		StatsInit(localClientNum);
		NotifyMsg("[Notice] Stats initialisées manuellement pour le client %d\n", localClientNum);
	}

	// 3. Appeler l'original mais intercepter les erreurs LUI juste après
	_hooks.LiveStorage_BeginGame_h.stub<void>(localClientNum);

	NotifyMsg("[Notice] <LiveStorage_BeginGame> original stub executed.\n");
}
...
//++++++++++++++++++++++++++++++
// en : Every frame execution function (with FULL authentication)
// ja : 毎フレーム実行関数 ( 完全認証 )
//++++++++++++++++++++++++++++++
...
void PatchLive_IsInSystemLinkLobby_Return1()
{
	__try {
		if (_adr.Live_IsInSystemlinkLobby) {
			uintptr_t addr = CalcPtr(_adr.Live_IsInSystemlinkLobby);
			if (addr && addr > _ImageBase) {
				DWORD oldProtect;
				VirtualProtect((void*)addr, 6, PAGE_EXECUTE_READWRITE, &oldProtect);
				// mov eax, 1; ret
				uint8_t patch[] = { 0xB8, 0x01, 0x00, 0x00, 0x00, 0xC3 };
				memcpy((void*)addr, patch, sizeof(patch));
				VirtualProtect((void*)addr, 6, oldProtect, &oldProtect);
				FlushInstructionCache(GetCurrentProcess(), (void*)addr, 6);
				NotifyMsg("[SystemLink] Patched to return 1\n");
			}
		}
	}
	__except (EXCEPTION_EXECUTE_HANDLER) {}
...
void R_EndFrame_d()
{
	if (!_frameCountEnd)
	{
		...
		{
		case 5:  // Frame 5 - Authentification complète
		{
			NotifyMsg("[Notice] Applying FULL authentication at frame %d\n", _elapsedFrameCount);


			__try {
				PatchLive_IsInSystemLinkLobby_Return1();  // ou HookLive_IsInSystemLinkLobby()
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] SystemLink lobby fix failed\n");
			}


			XUID xuid;
			xuid.RandomXUID();
			NotifyMsg("[Success] XUID generated: %llu\n", xuid.m_id);

			// =========================================================
			// PLATFORM PATCHES
			// =========================================================
			__try {
				uintptr_t signinState = CalcPtr(_adr.xenonUserData_m_guardedUserData_signinState);
				if (signinState) {
					utils::hook::set<int>(signinState, 2);
					utils::hook::set<bool>(signinState + 0x2D0, true);
					NotifyMsg("[Success] Platform signinState patched\n");
				}

				if (_adr.CurrentRegion_IssueFix1) {
					utils::hook::nop(CalcPtr(_adr.CurrentRegion_IssueFix1), 5);
				}
				if (_adr.CurrentRegion_IssueFix2) {
					utils::hook::nop(CalcPtr(_adr.CurrentRegion_IssueFix2), 5);
				}
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] Platform patches failed\n");
			}
			...
			__try {
				...
				NotifyMsg("[Notice] Forcing stats initialization (only online stats for controller 0)...\n");

				// Afficher les adresses pour debug
				uintptr_t statsAddr0online = CalcPtr(_adr.controllerStatData) + (controllerIndexSize * 0) + (statsSourceSize * StatsSource::STATS_ONLINE);
				uintptr_t statsAddr0offline = CalcPtr(_adr.controllerStatData) + (controllerIndexSize * 0) + (statsSourceSize * StatsSource::STATS_OFFLINE);
				uintptr_t statsAddr1online = CalcPtr(_adr.controllerStatData) + (controllerIndexSize * 1) + (statsSourceSize * StatsSource::STATS_ONLINE);
				uintptr_t statsAddr1offline = CalcPtr(_adr.controllerStatData) + (controllerIndexSize * 1) + (statsSourceSize * StatsSource::STATS_OFFLINE);

				NotifyMsg("[Debug] Stats addr 0online: 0x%llx\n", statsAddr0online);
				NotifyMsg("[Debug] Stats addr 0offline: 0x%llx\n", statsAddr0offline);
				NotifyMsg("[Debug] Stats addr 1online: 0x%llx\n", statsAddr1online);
				NotifyMsg("[Debug] Stats addr 1offline: 0x%llx\n", statsAddr1offline);

				// Forcer uniquement les stats qui existent : online pour controller 0 (et éventuellement 1)
				utils::hook::set<char>(statsAddr0online, 1);
				// Ne pas forcer les stats offline
				// utils::hook::set<char>(statsAddr0offline, 1); // commenté
				// Si on veut aussi controller 1 online (optionnel)
				// utils::hook::set<char>(statsAddr1online, 1);
				// utils::hook::set<char>(statsAddr1offline, 1); // commenté

				// Initialiser les stats pour controller 0 online seulement
				LiveStorage_StatsInit(0, 1, 0, StatsSource::STATS_ONLINE);
				// Ne pas initialiser les autres
				// LiveStorage_StatsInit(0, 1, 0, StatsSource::STATS_OFFLINE);
				// LiveStorage_StatsInit(1, 1, 0, StatsSource::STATS_ONLINE);
				// LiveStorage_StatsInit(1, 1, 0, StatsSource::STATS_OFFLINE);

				// Vérifier le résultat
				bool stats0online = LiveStorage_DoWeHaveStatsForSource_f(0, StatsSource::STATS_ONLINE);
				bool stats0offline = LiveStorage_DoWeHaveStatsForSource_f(0, StatsSource::STATS_OFFLINE);
				bool stats1online = LiveStorage_DoWeHaveStatsForSource_f(1, StatsSource::STATS_ONLINE);
				bool stats1offline = LiveStorage_DoWeHaveStatsForSource_f(1, StatsSource::STATS_OFFLINE);

				NotifyMsg("[Stats] Status - 0online:%d, 0offline:%d, 1online:%d, 1offline:%d\n",
					stats0online, stats0offline, stats1online, stats1offline);

				// Note: les stats offline et controller 1 sont censés être inexistants, donc on ne s'inquiète pas s'ils sont à 0.
				if (!stats0online) {
					NotifyMsg("[WARNING] Stats online for controller 0 failed, retrying...\n");
					// Réessayer...
					uintptr_t statsAddr0online = CalcPtr(_adr.controllerStatData) + (controllerIndexSize * 0) + (statsSourceSize * 0);
					DWORD oldProtect;
					VirtualProtect((void*)statsAddr0online, 1, PAGE_READWRITE, &oldProtect);
					*(char*)statsAddr0online = 1;
					VirtualProtect((void*)statsAddr0online, 1, oldProtect, &oldProtect);
					LiveStorage_StatsInit(0, 1, 0, StatsSource::STATS_ONLINE);
					stats0online = LiveStorage_DoWeHaveStatsForSource_f(0, StatsSource::STATS_ONLINE);
					NotifyMsg("[Notice] Second attempt - stats online: %s\n", stats0online ? "OK" : "FAILED");
				}

				NotifyMsg("[Success] Stats initialization complete (online only).\n");
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] Stats initialization failed\n");
			}

			// =========================================================
			// PROFILE PATCHES - Controller 0
			// =========================================================
			__try {
				XenonUserData* profile = Live_GetUserData(0);
				if (profile) {
					profile->signinState = 2;
					profile->xuid = xuid.m_id;
					profile->platformId = xuid.m_id;

					strncpy_s(profile->gamertag, "Player", _TRUNCATE);
					strncpy_s(profile->platformGamertag, "Player", _TRUNCATE);
					strncpy_s(profile->fullGamertag, "Player", _TRUNCATE);

					char xuid_str[32];
					sprintf_s(xuid_str, "%llu", xuid.m_id);
					strncpy_s(profile->xuidString, xuid_str, sizeof(profile->xuidString));
					strncpy_s(profile->platformIdString, xuid_str, sizeof(profile->platformIdString));

					NotifyMsg("[Success] Profile 0 patched with XUID: %s\n", xuid_str);
				}
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] Profile 0 patch failed\n");
			}

			// =========================================================
			// PROFILE PATCHES - Controller 1
			// =========================================================
			__try {
				XenonUserData* profile2 = Live_GetUserData(1);
				if (profile2) {
					profile2->signinState = 2;
					profile2->xuid = xuid.m_id + 1;
					profile2->platformId = xuid.m_id + 1;

					strncpy_s(profile2->gamertag, "Player2", _TRUNCATE);
					strncpy_s(profile2->platformGamertag, "Player2", _TRUNCATE);
					strncpy_s(profile2->fullGamertag, "Player2", _TRUNCATE);

					char xuid_str2[32];
					sprintf_s(xuid_str2, "%llu", xuid.m_id + 1);
					strncpy_s(profile2->xuidString, xuid_str2, sizeof(profile2->xuidString));
					strncpy_s(profile2->platformIdString, xuid_str2, sizeof(profile2->platformIdString));

					NotifyMsg("[Success] Profile 1 patched\n");
				}
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] Profile 1 patch failed\n");
			}
			...
			// =========================================================
			__try {
				if (_adr.Live_IsSignedIn) {
					utils::hook::jump(CalcPtr(_adr.Live_IsSignedIn), Live_IsSignedIn_d);
					NotifyMsg("[Success] Live_IsSignedIn patched\n");
				}

				if (_adr.Live_IsUserSignedIn) {
					utils::hook::jump(CalcPtr(_adr.Live_IsUserSignedIn), Live_IsUserSignedIn_d);
					NotifyMsg("[Success] Live_IsUserSignedIn patched\n");
				}

				if (_adr.Live_IsUserSignedInToBnet) {
					utils::hook::jump(CalcPtr(_adr.Live_IsUserSignedInToBnet), Live_IsUserSignedInToBnet_d);
					NotifyMsg("[Success] Live_IsUserSignedInToBnet patched\n");
				}

				if (_adr.Live_IsUserSignedInToDemonware) {
					utils::hook::jump(CalcPtr(_adr.Live_IsUserSignedInToDemonware), Live_IsUserSignedInToDemonware_d);
					NotifyMsg("[Success] Live_IsUserSignedInToDemonware patched\n");
				}

				if (_adr.dwGetLogOnStatus) {
					utils::hook::jump(CalcPtr(_adr.dwGetLogOnStatus), dwGetLogOnStatus_d);
					NotifyMsg("[Success] dwGetLogOnStatus patched\n");
				}
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] Function patches failed\n");
			}

			// =========================================================
			// CONTENT ENUMERATION PATCH
			// =========================================================
			__try {
				uintptr_t contentEnumAddr = CalcPtr(_adr.s_isContentEnumerationFinished);
				if (contentEnumAddr && contentEnumAddr > _ImageBase) {
					bool currentValue = *(bool*)contentEnumAddr;
					NotifyMsg("[Debug] s_isContentEnumerationFinished current value: %d at 0x%llx\n",
						currentValue, contentEnumAddr);
					*(bool*)contentEnumAddr = true;
					NotifyMsg("[Success] s_isContentEnumerationFinished set to 1\n");
				}
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] Failed to set s_isContentEnumerationFinished\n");
			}
			...
			// =========================================================
			__try {
				NotifyMsg("[Notice] Executing Cbuf commands...\n");
				if (Cbuf_AddText("MTQLNMTRLS 1;")) NotifyMsg("[Success] Executed: MTQLNMTRLS 1\n");
				if (Cbuf_AddText("LKSTRMKTML 0;")) NotifyMsg("[Success] Executed: LKSTRMKTML 0\n");
				if (Cbuf_AddText("PPMTTRNQM 0;")) NotifyMsg("[Success] Executed: PPMTTRNQM 0\n");
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] Failed to execute Cbuf commands\n");
			}

			// =========================================================
			// GAMER PROFILE INITIALIZATION
			// =========================================================
			__try {
				GamerProfile_LogInProfile(0);
				LoadSavedAchievements();
				NotifyMsg("[Success] Gamer profile initialized\n");
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				NotifyMsg("[ERROR] Gamer profile init failed\n");
			}

			_frameCountEnd = true;
			NotifyMsg("** SUCCESS ** FULL authentication applied at frame %d!\n", _elapsedFrameCount);
			break;
		}
...
//++++++++++++++++++++++++++++++
// en : Inject Lua script to unlock all weapons and attachments
// ja : すべての武器とアタッチメントをアンロックするLuaスクリプトを注入
//++++++++++++++++++++++++++++++
//++++++++++++++++++++++++++++++
// en : Inject Lua script to unlock all weapons and attachments
// ja : すべての武器とアタッチメントをアンロックするLuaスクリプトを注入
...
//++++++++++++++++++++++++++++++
// en : Game setup (BASE ABSOLUE - RIEN D'AUTRE)
// ja : ゲームセットアップ ( 絶対ベース - 他は何もなし )
//++++++++++++++++++++++++++++++
void GameSetup()
{
	NotifyMsg("=== GameSetup Start ===\n");

	// Hook Dvar_RegisterBool
	uintptr_t addr = CalcPtr(_adr.Dvar_RegisterBool);
	if (addr && addr > _ImageBase) {
		__try {
			_hooks.Dvar_RegisterBool_h.create(addr, Dvar_RegisterBool_d);
			NotifyMsg("[Success] Hooked Dvar_RegisterBool\n");
		}
		__except (EXCEPTION_EXECUTE_HANDLER) {
			NotifyMsg("[ERROR] Failed to hook Dvar_RegisterBool\n");
		}
	}

	// Hook LUI_COD_LuaCall_IsPremiumPlayer
	if (_adr.LUI_COD_LuaCall_IsPremiumPlayer) {
		__try {
			uintptr_t premiumAddr = CalcPtr(_adr.LUI_COD_LuaCall_IsPremiumPlayer);
			if (premiumAddr && premiumAddr > _ImageBase) {
				_hooks.LUI_COD_LuaCall_IsPremiumPlayer_h.create(premiumAddr, LUI_COD_LuaCall_IsPremiumPlayer_d);
				NotifyMsg("[Success] Hooked LUI_COD_LuaCall_IsPremiumPlayer to return true\n");
			}
		}
		__except (EXCEPTION_EXECUTE_HANDLER) {
			NotifyMsg("[ERROR] Failed to hook IsPremiumPlayer\n");
		}
	}
	...
	if (_adr.DB_LoadXFile) {
		uintptr_t addr = CalcPtr(_adr.DB_LoadXFile);
		if (addr && addr > _ImageBase) {
			_hooks.DB_LoadXFile_h.create(addr, DB_LoadXFile_d);
			NotifyMsg("[Success] Hooked DB_LoadXFile\n");
		}
	}


	// Hook LUI_CoD_LuaCall_OfflineDataFetched - Version avec offset
	__try {
		// Offset calculé: 0x6516150 (à partir de vos logs)
		uintptr_t offlineAddr = _ImageBase + 0x6516150;

		if (offlineAddr && offlineAddr > _ImageBase) {
			_hooks.LUI_CoD_LuaCall_OfflineDataFetched_h.create(offlineAddr, LUI_CoD_LuaCall_OfflineDataFetched_d);
			NotifyMsg("[Success] Hooked LUI_CoD_LuaCall_OfflineDataFetched at 0x%llx (offset: 0x6516150)\n", offlineAddr);
		}
		else {
			NotifyMsg("[ERROR] Invalid address for LUI_CoD_LuaCall_OfflineDataFetched\n");
		}
	}
	__except (EXCEPTION_EXECUTE_HANDLER) {
		NotifyMsg("[ERROR] Exception while hooking LUI_CoD_LuaCall_OfflineDataFetched\n");
	}
	/*
	// Hook LiveStorage_StatsInit (déplacé hors du __except)
	__try {
		uintptr_t statsAddr = CalcPtr(0x7FF6B0723620); // Adresse de LiveStorage_StatsInit

		if (statsAddr && statsAddr > _ImageBase) {
			_hooks.LiveStorage_StatsInit_h.create(statsAddr, LiveStorage_StatsInit_d);
			NotifyMsg("[Success] Hooked LiveStorage_StatsInit at 0x%llx\n", statsAddr);
		}
		else {
			NotifyMsg("[ERROR] Invalid address for LiveStorage_StatsInit\n");
		}
	}
	__except (EXCEPTION_EXECUTE_HANDLER) {
		NotifyMsg("[ERROR] Failed to hook LiveStorage_StatsInit\n");
	}
	*/



	// Injection du script Lua (ne fait rien avec nullptr, à modifier)
	// InjectWeaponUnlockerLua(nullptr); // À commenter pour l'instant

	NotifyMsg("=== GameSetup Complete ===\n");
}

// Fonction helper pour patcher une fonction pour qu'elle retourne true
...
void SetupProfile()
{
	...
	if (!file_exists(_profileIniPath.c_str())) {
		NotifyMsg("[WARNING] profile.ini not found, using defaults\n");
		_playerName = "Player1";
	}
	else {
	...
	_documentPath = (std::string)waFolderPath + "\\dlkmod";

	try {
		std::filesystem::create_directories(_documentPath);
	}
	catch (const std::exception& e) {
		NotifyMsg("[ERROR] Failed to create asset directory: %s\n", e.what());
	}
}
...
//++++++++++++++++++++++++++++++
// en : Splash screen loading (MINIMAL)
// ja : スプラッシュスクリーン読み込み ( 最小 )
//++++++++++++++++++++++++++++++
HANDLE WINAPI LoadImageA_d(HINSTANCE hInst, LPCSTR name, UINT type, int cx, int cy, UINT fuLoad)
{
	if (!_splashed)
	{
		...
		SetupProfile();  // Garder ça (lecture du profil)
		GameSetup();     // Maintenant seulement Dvar_RegisterBool
...
void Initialization()
{
	NotifyMsg("=== Initialization Start ===\n");
	NotifyMsg("ImageBase: 0x%llx\n", _ImageBase);

	// Test memory access
	__try {
		volatile char test = *(char*)_ImageBase;
		NotifyMsg("Game base readable: %c\n", test);
	}
	__except (EXCEPTION_EXECUTE_HANDLER) {
		NotifyMsg("[CRITICAL] Cannot read game memory!\n");
		return;
	}
	...
	if (_adr.R_EndFrame == 0) {
		NotifyMsg("[CRITICAL] R_EndFrame address is null!\n");

		return;
	}

	// Hook les fonctions EXE d'abord
	...
	NotifyMsg("=== Initialization Complete ===\n");
	Sleep(250);
